...
export default function EditFamily() {
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true);
    setError(null);
    // Store an unset birth type as null rather than an empty string,
    // so the care plan generator can tell it hasn't been chosen.
    const payload = { ...form, birth_type: form.birth_type || null };
    const { error } = await supabase.from('families').update(payload).eq('id', id);
    if (error) { setError(error.message); setSaving(false); return; }
    router.push(`/families/${id}`);
  }

  const input = {width:'100%',padding:'0.5rem 0.75rem',border:'1px solid #d1d5db',borderRadius:'0.5rem',fontSize:'0.875rem',outline:'none',boxSizing:'border-box',color:'#1a1a1a'};
    ...
    <div style={{minHeight:'100vh',background:'#faf6ef'}}>
      ...
      <main style={{maxWidth:'640px',margin:'0 auto',padding:'2rem 1.5rem'}}>
        <div style={{marginBottom:'1.5rem'}}>
          <a href={`/families/${id}`} style={{color:'#7a9582',textDecoration:'none',fontSize:'0.875rem',fontWeight:'500'}}>← Back to Family</a>
        ...
        <div style={{background:'white',borderRadius:'0.75rem',border:'1px solid #e5e7eb',padding:'1.5rem'}}>
          <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:'1.25rem'}}>
            ...
            ].map(([name,lbl,type,req]) => (
              <div key={name}>
                <label style={label}>{lbl}{req && ' *'}</label>
                <input type={type} name={name} required={req} value={form[name]} onChange={e=>setForm({...form,[name]:e.target.value})} style={input}/>
              </div>
            ))}
            <div>
              <label style={label}>Birth Date</label>
              <input type="date" value={form.birth_date} onChange={e=>setForm({...form,birth_date:e.target.value})} style={input}/>
            </div>
            <div>
              ...
              <p style={{fontSize:'0.75rem',color:'#6b7280',margin:'0.35rem 0 0'}}>Needed before a care plan can be generated.</p>
            </div>
            <div>
              <label style={label}>Notes</label>
              <textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} rows={3} style={{...input,resize:'vertical'}}/>
            </div>
            <div style={{display:'flex',gap:'0.75rem',paddingTop:'0.5rem'}}>
              <button type="submit" disabled={saving} style={{flex:1,padding:'0.625rem 1rem',background:'#7a9582',color:'white',border:'none',borderRadius:'0.5rem',fontSize:'0.875rem',fontWeight:'600',cursor:'pointer'}}>
